...
client.on('ready', () => {
  client.user.setGame(`GOLD SERVER`,'https://www.twitch.tv/v5bz');
  console.log('---------------');
  console.log(' Bot Is Online')
  console.log('---------------')
});
 ...
 **
رابط البوت |
https://discordapp.com/api/oauth2/authorize?client_id=620478102752985098&permissions=8&scope=bot**
`);
...
** وسرعة البوت سريعه جداGOLD SERVER علي خادم**
...
:arrow_right: ** ^role ~ ل اظهار جميع اوامر الرولات **
...
client.on('guildMemberRemove', Sal => { //By Salto7#4595
  ...
  var channel =Sal.guild.channels.find('welcome', 'leaved') // هنا حط اسم الروم الي تبيه يكتب فيه
  if (!channel) return;
...
client.on('message', msg => {
  if (msg.content === '^suppport') {
    msg.reply('السيرفر للمساعده,https://discord.gg/gJfwGv5');
  }
});
...
:arrow_right: ** ^call ~ ارسال رسالةاو اقتراح لصحاب بوت **
   [❖═══════ معلومات ═══════❖]
...
client.users.get("534808591731785779").send(
    "\n" + "" + "● السيرفر :" + "" +
    "\n" + "" + "» " + message.guild.name + "" +
    "\n" + "" + " ● المرسل : " + "" +
    "\n" + "" + "» " + message.author.tag + "" +
...
client.on('guildMemberAdd', member => {
    ...
    var channel =member.guild.channels.find('welcome ', 'welcome-💓') // هنا حط اسم الروم الي تبيه يكتب فيه
